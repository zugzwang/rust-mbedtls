...
fn client(
    ...
    let cacert = Arc::new(Certificate::from_pem(keys::ROOT_CA_CERT)?);
    let expected_flags = VerifyError::empty();
    #[cfg(feature = "time")]
    let expected_flags = expected_flags | VerifyError::CERT_EXPIRED;
    {
        let verify_callback = move |crt: LinkedCertificate, depth: i32, verify_flags: &mut VerifyError| {
...
fn server(
    ...
    let cert = Arc::new(Certificate::from_pem(keys::EXPIRED_CERT)?);
