...
function build_and_sign() {
    ...
    rm -f ./$binary_name.sgxs
    ftxsgx-elf2sgxs ./$binary_name --heap-size 409600 --stack-size 409600 --threads $threads --debug > /dev/null
    sgxs-sign ./$binary_name.sgxs ./sigstruct.bin --key ./private.pem -p $isvprodid -v $isvsvn > /dev/null
...
echo "Debug build: ./target/x86_64-fortanix-unknown-sgx/debug/$binary_name.sgxs"
echo ""
echo "You can copy this to a machine running Fortanix Node Agent and run it via"
echo "ftxsgx-runner ./target/x86_64-fortanix-unknown-sgx/debug/$binary_name.sgxs"
