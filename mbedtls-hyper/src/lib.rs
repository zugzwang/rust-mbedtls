...
mod tests {
    ...
    fn test_verify() {
        ...
        let verify_callback = |_crt: LinkedCertificate, _depth: i32, verify_flags: &mut VerifyError| {
        ...
        #[cfg(not(target_env = "sgx"))]
        set_global_debug_threshold(1);
        
        config.set_ca_list(Arc::new(Certificate::from_pem(ROOT_CA_CERT).unwrap()), None);
    ...
    fn test_hyper_server() {
        ...
        let cert = Arc::new(Certificate::from_pem(PEM_CERT).unwrap());
